...
<template>
  <button class="btn-sm w-8 h-8 btn" @click="open = !open">
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="hover:rotate-180 transition-all duration-500 w-6 h-6 min-w-[24px] min-h-[24px]" stroke-width="1.5" stroke="white" >
      ...
      <div class="modal-box min-w-fit" @click.stop>
        ...
        <div class="flex w-full items-center justify-between mt-8">
          ...
          <select v-model="config.compile.mode" class="select select-bordered">
            ...
            <option value="instantly" v-text="'моментальный'" />
          </select>
        </div>
      </div>
    </div>
